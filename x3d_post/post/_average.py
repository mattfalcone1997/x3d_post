...
class _AVG_base(CommonData, stathandler_base, ABC):
    ...
    def _extract_avg(self, it, path, it0=None):
        ...
        if self._has_data(path, 'pp_mean'):
            self.pp_data = self._extract_ppmean(path, it, it0)
    ...
    def check_PhyTime(self, PhyTime):
        ...
        err_msg = KeyError(
            "PhyTime provided is not in the CHAPSim_AVG datastruct, recovery impossible")
    ...
    def _extract_uuumean(self, path, it, it0):
        ...
        if hasattr(it, '__iter__'):
            items = list(
                chain(*[np.array([0, 6, 8])+10*i for i in range(len(it))]))
            l = l[items]
        else:
            l = l[[0, 6, 8]]
        geom = fp.GeomHandler(self.metaDF['itype'])
    ...
    def _extract_ppmean(self, path, it, it0):
        l = self._get_data(path, 'pp_mean', None, it, 1)
        if it0 is not None:
            l0 = self._get_data(path, 'pp_mean', None, it0, 1)
            it_ = self._get_nstat(it)
            it0_ = self._get_nstat(it0)
        ...
        pp_data = self._flowstruct_class(coorddata,
                                         l,
                                         index=index)
        return pp_data
...
class _AVG_developing(_AVG_base):
    ...
    def _tau_calc(self, PhyTime):
        ...
        mu_star = 1.0
        tau_star = -mu_star * \
            (u_velo[-1, :] - u_velo[-2, :])/(ycoords[-1]-ycoords[-2])
...
class x3d_avg_z(_AVG_developing, stat_z_handler):
    ...
    def _get_uplus_yplus_transforms(self, PhyTime, x_val):
        ...
        def x_transform(
            y): return y[1:]/delta_v[x_index] if isinstance(y, np.ndarray) else y/delta_v[x_index]
        def y_transform(
            u): return u[1:]/u_tau[x_index] if isinstance(u, np.ndarray) else u/u_tau[x_index]
    ...
    def plot_flow_wall_units(self, x_vals, plot_sublayer=True, PhyTime=None, fig=None, ax=None, line_kw: dict = None, **kwargs):
        ...
        x_labels = [self.Domain.create_label(
            r"$x = %.3g$" % x) for x in x_vals]
        for i, x in enumerate(x_vals):
            ...
            x_transform, y_transform = self._get_uplus_yplus_transforms(
                PhyTime, x)
        ...
        if not r"$\bar{u}^+=y^+$" in labels and plot_sublayer:
            ...
            ax.cplot(u_plus_array, u_plus_array,
                     label=r"$\bar{u}^+=y^+$", color='r', linestyle='--')
    ...
    def _get_uuplus_yplus_transforms(self, PhyTime, x_val, y_transform):
        ...
        if y_transform is None:
            def y_transform(u): return u[1:]/(u_tau[x_index]*u_tau[x_index]) \
    ...
    def plot_Reynolds_x(self, comp, y_vals_list, y_mode='half-channel', PhyTime=None, fig=None, ax=None, line_kw=None, **kwargs):
        ...
        if y_vals_list == 'max':
            ...
            fig, ax = self.uu_data.plot_line_max(
                comp, 'x', time=PhyTime, fig=fig, ax=ax, line_kw=line_kw, **kwargs)
        else:
    ...
    def plot_flatness(self, comp, axis_vals, direction='y', y_mode_wall=False, x_val=0, norm=True, PhyTime=None, fig=None, ax=None, line_kw=None, **kwargs):
        ...
        labels = [self.Domain.create_label(
            rf"${unit} = %.3g$" % x) for x in axis_vals]
        ...
        if norm:
            ax.set_ylabel(
                r"$\overline{%s'^4}/\overline{%s'^2}^2$" % (comp[0], comp[0]))
        else:
    ...
    def plot_skewness(self, comp, axis_vals, direction='y', y_mode_wall=False, x_val=0, norm=True, PhyTime=None, fig=None, ax=None, line_kw=None, **kwargs):
        ...
        if norm:
            ax.set_ylabel(
                r"$\overline{%s'^3}/\overline{%s'^2}^{3/2}$" % (comp[0], comp[0]))
        else:
...
class x3d_avg_xz(_AVG_base, stat_xz_handler):
    ...
    def _get_uplus_yplus_transforms(self, PhyTime):
        ...
        def x_transform(
            y): return y[1:]/delta_v if isinstance(y, np.ndarray) else y/delta_v
        def y_transform(u): return u[1:] / \
            u_tau if isinstance(u, np.ndarray) else u/u_tau
    ...
    def _get_uuplus_y_plus_transforms(self, PhyTime, y_transform):
        ...
        if y_transform is None:
            def y_transform(
                u): return u[1:]/(u_tau*u_tau) if isinstance(u, np.ndarray) else u/(u_tau*u_tau)
        else:
            def y_transform(u): return y_transform(
                u[1:])/(u_tau*u_tau) if isinstance(u, np.ndarray) else u/(u_tau*u_tau)
        return x_transform, y_transform
    ...
    def plot_lumley(self, y_locs, wall_units=True, plot_limits=True, PhyTime=None, fig=None, ax=None, line_kw=None, **kwargs):

        fig, ax = create_fig_ax_with_squeeze(fig, ax, **kwargs)
        if wall_units:
            y_indices = self.Wall_Coords(PhyTime).index_calc('y', y_locs)
        else:
            y_indices = self.CoordDF.index_calc('y', y_locs)
...
class x3d_avg_xzt(_AVG_developing, stat_xzt_handler, x3d_avg_xz, CommonTemporalData):
    ...
    def _hdf_extract(self, fn, key=None):
        ...
        self.mean_data = fp.FlowStruct1D_time.from_hdf(
            fn, key=key+'/mean_data')
        self.uu_data = fp.FlowStruct1D_time.from_hdf(fn, key=key+'/uu_data')

        h5_obj = fp.hdfHandler(fn, 'r', key=key)
        if 'uuu_data' in h5_obj.keys():
            self.uuu_data = fp.FlowStruct1D_time.from_hdf(
                fn, key=key+'/uuu_data')

        if 'uuuu_data' in h5_obj.keys():
            self.uuuu_data = fp.FlowStruct1D_time.from_hdf(
                fn, key=key+'/uuuu_data')
    ...
    def Wall_Coords(self, axis_val):
        _, delta_v = self.wall_unit_calc(axis_val)
        return self.CoordDF / delta_v
    ...
    def plot_mean_flow(self, comp, PhyTimes, fig=None, ax=None, line_kw=None, **kwargs):
        ...
        for time in PhyTimes:
            ...
            fig, ax = super().plot_mean_flow(comp, time, fig=fig,
                                             ax=ax, line_kw=line_kw, **kwargs)
    ...
    def plot_Reynolds(self, comp, PhyTimes, fig=None, ax=None, line_kw=None, **kwargs):
        ...
        for time in PhyTimes:
            ...
            fig, ax = super().plot_Reynolds(comp, PhyTime=time,
                                            fig=fig, ax=ax, line_kw=line_kw, **kwargs)
    ...
    def plot_Reynolds_x(self, comp, y_vals_list, y_mode='half-channel', fig=None, ax=None, line_kw=None, **kwargs):
        ...
        if y_vals_list == 'max':
            ...
            fig, ax = self.uu_data.plot_line_time_max(
                comp, fig=fig, ax=ax, line_kw=line_kw, **kwargs)
        else:
            msg = "This method needs to be reimplemented only max can currently be used"
            raise NotImplementedError(msg)
