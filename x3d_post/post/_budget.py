...
class budgetBase(CommonData, ABC):
    ...
    def _hdf_extract(self, fn, key=None):
        ...
        self.budget_data = self._flowstruct_class.from_hdf(
            fn, key=key+'/budget_data')
    ...
    def _create_budget_axes(self, x_list, fig=None, ax=None, **kwargs):
        ...
        kwargs = update_subplots_kw(
            kwargs, gridspec_kw=gridspec_kw, figsize=figsize)
        fig, ax, single_input = create_fig_ax_without_squeeze(
            *ax_size, fig=fig, ax=ax, **kwargs)
    ...
    def set_symbols(self, symbols: dict[str, str], fail_on_absent=False):
        for k, v in symbols.items():
            if k in self.budget_data.inner_index:
                self._symbols[k] = v
            elif fail_on_absent:
                raise ValueError(f"{k} not in budget: "
                                 f"{self.budget_data.inner_index}")

    def set_styles(self, styles: dict[str, dict], fail_on_absent=False):
...
class x3d_budget_z(ReynoldsBudget_base, budgetBase, stat_z_handler):
    ...
    def plot_budget(self, x_list, budget_terms=None, wall_units=True, fig=None, ax=None, line_kw=None, **kwargs):
        ...
        fig, ax, single_input = self._create_budget_axes(
            x_list, fig=fig, ax=ax, **kwargs)
        line_kw = update_line_kw(line_kw)

        for i, x_loc in enumerate(x_list):
            x = self.CoordDF.index_calc('x', x_loc)[0]
            y_plus, budget_scale = self._wallunit_generator(
                x, None, wall_units)
            for comp in budget_terms:
                line_kw_copy = line_kw.copy()
                line_kw_copy['label'] = self.title_with_math(
                    self._symbols.get(comp, comp))
                if comp in self._styles:
                    line_kw_copy.update(self._styles.get(comp))
                ...
                fig, ax[i] = self.budget_data.plot_line(comp, 'y', x_loc, time=None, channel_half=True,
                                                        ...
                                                        fig=fig, ax=ax[i], line_kw=line_kw_copy)
        ...
        fig.clegend(handles, labels, loc='upper center',
    ...
    def plot_integral_budget(self, budget_terms, PhyTime=None, fig=None, ax=None, line_kw=None, **kwargs):
        ...
        for comp in budget_terms:
            ...
            int_budget = 0.5 * \
                self.Domain.Integrate_tot(self.Coord_ND_DF, budget_term)
    ...
    def plot_budget(self, PhyTime=None, budget_terms=None, wall_units=True, fig=None, ax=None, line_kw=None, **kwargs):
        ...
        for comp in budget_terms:
            ...
            fig, ax = self.budget_data.plot_line(comp, time=PhyTime,
                                                 ...
                                                 transform_ydata=budget_scale,
                                                 channel_half=True,
                                                 fig=fig, ax=ax, line_kw=line_kw_copy)
...
class _pstrain_base(CommonData, stathandler_base):
    def __init__(self, *args, from_hdf=False, **kwargs):
        if not from_hdf:
            self._pstrain_extract(*args, **kwargs)
        else:
            self._hdf_extract(*args, **kwargs)
    ...
    def set_symbols(self, symbols: dict):
        for k, v in symbols.items():
            if k in self.pstrain_data.inner_index:
                self._symbols[k] = v
    ...
    def save_hdf(self, fn, mode, key=None):
        ...
        self.pstrain_data.to_hdf(fn, 'a', key=key+'/pstrain_data')

        return hdf_obj

    def _extract_pstrain(self, path, it, it0):

        l = self._get_data(path, 'pdudxq_mean', None, it, 4)

        if it0 is not None:
            l0 = self._get_data(path, 'pdudxq_mean', None, it0, 4)
            it_ = self._get_nstat(it)
            it0_ = self._get_nstat(it0)
        ...
        comps = ['pdudx_neg', 'pdwdz_neg', 'pdudx_pos', 'pdwdz_pos']
        index = self._get_index(it, comps)

        pdudx = self._flowstruct_class(coorddata,
                                       l,
                                       index=index)
        return pdudx
    ...
    def _check_terms(self, terms):
        if terms is None:
            return self.pdudx_data.inner_index

        if not all(term in self.pdudx_data.inner_index for term in terms):
            raise ValueError("Invalid term")

        return terms


class x3d_pstrain_xz(_pstrain_base, stat_xz_handler):
    def _pstrain_extract(self, it, path, it0=None):
        self._avg_data = self._module._avg_xz_class(it, path, it0)
        self.pdudx_data = self._extract_pstrain(path, it, it0)
    ...
    def _hdf_extract(self, fn, key):
        ...
        self._avg_data = self._module._avg_xz_class.from_hdf(
            fn, key=key+'/avg_data')

        self.pdudx_data = fp.FlowStruct1D.from_hdf(fn, key=key+'/pdudx_data')
    ...
    def plot_line(self, PhyTime=None, terms=None, wall_units=True, fig=None, ax=None, line_kw=None, **kwargs):
        fig, ax = create_fig_ax_with_squeeze(fig, ax, **kwargs)
        terms = self._check_terms(terms)
        line_kw = update_line_kw(line_kw)
        ...
        for comp in terms:
            ...
            if comp in self._styles:
                line_kw_copy.update(self._styles.get(comp))
            fig, ax = self.pdudx_data.plot_line(comp, time=PhyTime,
                                                transform_xdata=y_plus,
                                                transform_ydata=budget_scale,
                                                channel_half=True,
                                                fig=fig, ax=ax, line_kw=line_kw_copy)
...
class momentum_balance_base(budgetBase):
    ...
    def _budget_extract(self, comp):
        ...
        budget_index = ['advection', 'pressure gradient',
                        'viscous stresses', 'reynolds stresses']
        phystring_index = [None]*4

        data = self._flowstruct_class(self.avg_data._coorddata, budget_array, index=[
                                      phystring_index, budget_index])

        return data

    def _integrate_budget(self, array, from_freestream=True):
        y = self.budget_data.CoordDF['y']
        if self.Domain.is_channel:
            y_mid = (array.shape[0]+1)//2
            array_half = array[:y_mid]
            y_half = y[:y_mid]

            if from_freestream:
                array_half = array_half[::-1]
                y_half = y_half[::-1]
...
class x3d_mom_balance_xz(momentum_balance_base, budgetBase):
    ...
    def _viscous_extract(self, comp):
        ...
        U_comp = self.mean_data[comp]
        dudy = fp.Grad_calc(self.mean_data.CoordDF, U_comp, 'y')
        d2udy2 = fp.Grad_calc(self.mean_data.CoordDF, dudy, 'y')
    ...
    def _turb_transport(self, comp):
        ...
        return -1*self.Domain.Grad_calc(self.uu_data.CoordDF, uv, 'y')
...
class x3d_mom_balance_xzt(x3d_mom_balance_xz, budget_xzt_base):
    ...
    def _budget_extract(self, comp):
        ...
        array_concat = [transient, advection,
                        pressure_grad, viscous, Reynolds_stress]

        budget_array = np.concatenate(array_concat, axis=0)

        comps = ['transient', 'advection', 'pressure gradient',
                 'viscous stresses', 'reynolds stresses']
    ...
    def plot_balance(self, times_list, budget_terms=None, fig=None, ax=None, line_kw=None, **kwargs):
        ...
        fig, ax, single_input = self._create_budget_axes(
            times_list, fig, ax, **kwargs)
        for i, time in enumerate(times_list):
            time = self.avg_data.check_PhyTime(time)
            fig, ax[i] = super().plot_balance(
                PhyTime=time, budget_terms=budget_terms, fig=fig, ax=ax[i], line_kw=line_kw)
    ...
    def plot_integrated_budget(self, times_list, budget_terms=None, fig=None, ax=None, line_kw=None, **kwargs):
        ...
        fig, ax, single_input = self._create_budget_axes(
            times_list, fig, ax, **kwargs)
        for i, time in enumerate(times_list):
            fig, ax[i] = super().plot_integrated_budget(
                PhyTime=time, budget_terms=budget_terms, fig=fig, ax=ax[i], line_kw=line_kw)
...
class _FIK_base(budgetBase):
    ...
    def _budget_extract(self):
        ...
        budget = fp.datastruct(budget_array, index=[
                               phystring_index, budget_index])
...
class _FIK_developing_base(_FIK_base):
    ...
    def plot(self, budget_terms=None, plot_total=True, PhyTime=None, fig=None, ax=None, line_kw=None, **kwargs):
        ...
        for comp in budget_terms:
            ...
            ax.cplot(xaxis_vals, budget_term, label=label, **line_kw_copy)
        if plot_total:
            ax.cplot(xaxis_vals, np.sum(self.budget_data.values,
                     axis=0), label="Total", **line_kw)
...
class x3d_FIK_z(_FIK_developing_base):
    ...
    def _inertia_extract(self):
        ...
        pressure_grad_x = self.Domain.Grad_calc(
            self.avg_data.CoordDF, pressure, 'x')
...
class x3d_FIK_xzt(_FIK_developing_base):
    ...
    def _inertia_extract(self):
        ...
        dp_prime_dx = dpdx - \
            self._integrate((1/REN)*d2u_dy2 - duv_dy, y_coords)
...
class Cf_Renard_base(budgetBase):
    ...
    def _kinetic_energy_y_extract(self):
        ...
        v_dKdy = (u_mean-u)*v_mean*fp.Grad_calc(self.CoordDF, u_mean, 'y')
        return v_dKdy/(u*u*u)
    ...
    def _plot_x(self, budget_terms=None, plot_total=True, PhyTime=None, fig=None, ax=None, line_kw=None, **kwargs):
        ...
        for comp in budget_terms:
            ...
            ax.cplot(xaxis_vals, budget_term, **line_kw_copy)
        if plot_total:
            ax.cplot(xaxis_vals, np.sum(self.budget_data.values,
                     axis=0), label="Total", **line_kw)
...
class x3d_Cf_Renard_z(Cf_Renard_base):
    def _budget_extract(self, separate_xy=False):
        ...
        diffusion = self._diffusion_x_extract()

        if not separate_xy:
            array_concat = [self._integrate(dissipation_y+dissipation_x),
                            self._integrate(production_y+production_x),
                            self._integrate(kinetic_x+kinetic_y+pressure),
                            self._integrate(diffusion)]
            budget_index = ['dissipation',
                            'production', 'kinetic', 'diffusion']
        else:
            array_concat = [self._integrate(dissipation_y),
                            self._integrate(dissipation_x),
                            self._integrate(production_y),
                            self._integrate(production_x),
                            self._integrate(kinetic_x),
                            self._integrate(kinetic_y),
                            self._integrate(pressure),
                            self._integrate(diffusion)]
    ...
    def _integrate(self, array):
        y = self.CoordDF['y']
        return 2.0*simps(array, y, axis=0)
    ...
    def _dissipation_x_extract(self):
        ...
        # dudx1 = fp.Grad_calc(self.CoordDF,u_mean,'x')
    ...
    def _production_x_extract(self):
        ...
        return - uu_mean*dudx/(u*u*u)

    def _diffusion_x_extract(self):
        u = self._scale_vel()
        u_mean = self.avg_data.mean_data['u']
        re = self.metaDF['re']
        ...
        dK2dx2 = fp.Grad_calc(self.CoordDF,
                              (u_mean-u)*dudx,
                              'x')
        y = self.CoordDF['y']
        return dK2dx2/(u*u*u)
    ...
    def _kinetic_energy_x_extract(self):
        ...
        u_dKdx = (u_mean-u)*u_mean*fp.Grad_calc(self.CoordDF, u_mean, 'x')
        y = self.CoordDF['y']
        return u_dKdx/(u*u*u)
...
class x3d_Cf_Renard_xzt(Cf_Renard_base):
    ...
    def _pressure_extract(self):
        ...
        dudy = fp.Grad_calc(self.CoordDF, self.avg_data.mean_data['u'], 'y')
        d2u_dy2 = fp.Grad_calc(self.CoordDF, dudy, 'y')
        dudt = np.gradient(u_mean, self.times, axis=-1)
        re = self.metaDF['re']
        dpdx = -(d2u_dy2/re - d_uv_dy) + dudt
        u_dpdx = u_mean*dpdx
        y = self.CoordDF['y']
        return -u_dpdx/(u*u*u)

    def _budget_extract(self, separate=False):
        dissipation_y = self._dissipation_y_extract()
        production_y = self._production_y_extract()
        kinetic_y = self._kinetic_energy_y_extract()
        pressure = self._pressure_extract()
        ...
        if separate:
            array_concat = [self._integrate(dissipation_y),
                            self._integrate(production_y),
                            self._integrate(kinetic_y+pressure),
                            self._integrate(transient)]
            budget_index = ['dissipation',
                            'production', 'kinetic', 'transient']
        else:
            array_concat = [self._integrate(dissipation_y),
                            self._integrate(production_y),
                            self._integrate(kinetic_y+transient+pressure)]
            budget_index = ['dissipation', 'production', 'kinetic']
    ...
    def _transient_extract(self):
        ...
        u_mean = self.avg_data.mean_data['u']-u
        times = self.avg_data.times
        dKdt = u_mean*np.gradient(u_mean, times, axis=1)
