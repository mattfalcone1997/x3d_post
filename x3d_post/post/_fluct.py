...
class _fluct_base(CommonData):
    ...
    def save_hdf(self, file_name, write_mode, key=None):
        ...
        self.fluct_data.to_hdf(file_name, key=key+'/fluct_data', mode='a')
    ...
    def plot_contour(self, comp, axis_vals, plane='xz', PhyTime=None, ref_loc=None, wall_units=True, fig=None, ax=None, contour_kw=None, **kwargs):
        ...
        if coord == 'y' and wall_units:
            if ref_loc is None:
                ref_loc = self._default_ref_loc
            int_vals = self.avg_data.get_coords_wall_units(
                coord, axis_vals, ref_loc)
            axis_vals = self.avg_data.Wall_Coords(
                ref_loc).get_true_coords('y', axis_vals)
            title_symbol = get_symbol('wall_initial')

        else:
            int_vals = axis_vals = self.CoordDF.get_true_coords(
                coord, axis_vals)
        ...
        fig, ax, axes_output = create_fig_ax_without_squeeze(
            len(axis_vals), fig=fig, ax=ax, **kwargs)

        for i, val in enumerate(int_vals):
            fig, ax1 = self.fluct_data.plot_contour(
                comp, plane, val, time=PhyTime, fig=fig, ax=ax[i], contour_kw=contour_kw)
            ...
            ax1.axes.set_title(r"$%s=%.2g$" %
                               (title_symbol, axis_vals[i]), loc='right')
    ...
    def plot_vector(self, plane, axis_vals, PhyTime=None, wall_units=True, spacing=(1, 1), scaling=1, x_split_list=None, fig=None, ax=None, quiver_kw=None, **kwargs):
        ...
        if coord == 'y' and wall_units:
            int_vals = self._avg_data.get_coords_wall_units(
                coord, axis_vals, 0)
            axis_vals = self._avg_data.Wall_Coords(
                0).get_true_coords('y', axis_vals)
            title_symbol = get_symbol('wall_initial')
...
class x3d_fluct_z(_fluct_base):
    def _fluct_extract(self, time_inst_data_list, avg_data=None, path='.', *args, **kwargs):
        ...
        for i, time_inst_data in enumerate(time_inst_data_list):
            if isinstance(time_inst_data, self._module._inst_z_class):
                ...
                if i == 0:
                    inst_data = self._module._inst_z_class(
                        time_inst_data, path=path, avg_data=avg_data, *args, **kwargs)
                else:
                    inst_data += self._module._inst_z_class(
                        time_inst_data, path=path, avg_data=avg_data, *args, **kwargs)
    ...
    def _hdf_extract(self, filename, key=None):
        ...
        self.avg_data = self._module._avg_z_class.from_hdf(
            filename, key=key+"/avg_data")
        self.fluct_data = fp.FlowStruct3D.from_hdf(
            filename, key=key+'/fluct_data')
    ...
    def _fluct_data_calc(self, inst_data, avg_data):
        ...
        assert len(
            avg_time) == 1, "In this context therecan only be one time in avg_data"
        fluct = np.zeros((len(inst_data.inst_data.index), *
                         inst_data.shape[:]), dtype=fp.rcParams['dtype'])
        j = 0
    ...
    def _get_avg_slice(self, inst_struct: fp.FlowStruct3D, mean_struct: fp.FlowStruct2D):
        ...
        y_indices = [np.argmin(abs(coords_avg['y'] - y))
                     for y in coords_inst['y']]
        x_indices = [np.argmin(abs(coords_avg['x'] - x))
                     for x in coords_inst['x']]
...
class x3d_fluct_xzt(_fluct_base):
    def _fluct_extract(self, time_inst_data_list, avg_data=None, path='.', *args, **kwargs):
        ...
        for i, time_inst_data in enumerate(time_inst_data_list):
            if isinstance(time_inst_data, self._module._inst_xzt_class):
                ...
                if i == 0:
                    inst_data = self._module._inst_xzt_class(
                        time_inst_data, path=path, avg_data=avg_data, *args, **kwargs)
                else:
                    inst_data += self._module._inst_xzt_class(
                        time_inst_data, path=path, avg_data=avg_data, *args, **kwargs)
    ...
    def _hdf_extract(self, filename, key=None):
        ...
        self.avg_data = self._module._avg_xzt_class.from_hdf(
            filename, key=key+"/avg_data")
        self.fluct_data = fp.FlowStruct3D.from_hdf(
            filename, key=key+'/fluct_data')
