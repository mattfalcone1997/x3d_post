...
class spectra_base(stathandler_base, CommonData):
    ...
    def _hdf_extract(self, fn, key=None):
        ...
        self.spectra_data = self._flowstruct_class.from_hdf(
            fn, key=key+'/spectra_data')
...
class x3d_spectra_xz(stat_xz_handler, spectra_base):
    ...
    def _get_spectra_data(self, it, path, it0=None, comps=None):
        ...
        comps_avail = list(set([l.replace('spectra_2d_', '')
                                .replace('.dat', ' ').split()[0]
                                for l in files]))
        if comps is None:
            comps = comps_avail
        ...
        for i, comp in enumerate(comps):
            ...
            if it0 is not None:
                fn = stathandler_base._get_stat_file_z(
                    path, 'spectra_2d_'+comp, it0)
        ...
        if self.Domain.is_channel:
            ...
            for i, comp in enumerate(index[1]):
                l[i] = self._apply_symmetry(
                    comp, l[i], 1, noflip=noflip, exclude=exclude)
    ...
    def _process_spectra(self, path, it, it0, l, comps, kx, kz):
        ...
        for i, comp in enumerate(comps):
            ...
            if comp == 'pdvdy':
                l[i] = 2*l[i]
            if comp == 'omega_y':
                l[i] = -dudx_mean['dudy'][None, :, None] * \
                    l[i]*kz[:, None, None]*1.0j
    ...
    def get_autocorrelation(self, comps=None, time=None):
        ...
        for i, comp in enumerate(comps):
            ...
            autocorr = irfft(ifft(data, axis=2, norm='forward'),
                             axis=0, norm='forward')
        ...
        delta_z = np.linspace(-z_coords[-1]/2-dz,
                              z_coords[-1]/2-dz, autocorr.shape[0])
        delta_x = np.linspace(-x_coords[-1]/2-dx,
                              x_coords[-1]/2-dx, autocorr.shape[2])
    ...
    def get_energy(self, comps=None):
        ...
        shape = (2*(self.spectra_data.shape[0]-1),
                 *self.spectra_data.shape[1:])
    ...
    def plot_spectra_2d(self, comp, coord, wavelength=False, premultiply=True, y_wall_units=True, time=None, contour_kw=None, fig=None, ax=None, **kwargs):
        ...
        c_transform = (lambda x: np.real(x*k_x*k_z)
                       ) if premultiply else np.real
    ...
    def plot_spectra_x(self, comp,
        ...
        x_transform = (lambda x: 2*np.pi/(x+x[1])) if wavelength else None
        c_transform = (lambda x: np.real(transform_cdata(x)*k_x)
                       ) if premultiply else np.real
...
class x3d_spectra_xzt(x3d_spectra_xz, CommonTemporalData):
    ...
    def _get_spectra_from_inst(self, it, path, spectra_level=1):
        ...
        nspectra = {1: 4, 2: 8}
        shape = (nspectra[spectra_level], self.NCL[2] //
                 2+1, self.NCL[1], (self.NCL[0]))
        ...
        if spectra_level >= 2:
            ...
            dvdy = fp.Grad_calc(self.avg_data.CoordDF,
                                fluct_data.fluct_data['v'], 'y')
            ...
            data[7] = -1.0j*dudx_mean['dudy'][None, :, None] * \
                k_z[:, None, None]*omega_spec
        ...
        if not show_positive:
            ...
            if 'levels' in contour_kw:
                if isinstance(contour_kw['levels'], int):
                    contour_kw['levels'] = np.linspace(
                        vmin, 0., contour_kw['levels'])
...
class x3d_autocorr_x(CommonData, stathandler_base):
    ...
    def _hdf_extract(self, fn, key=None):
        ...
        self.autocorr_data = self._flowstruct_class.from_hdf(
            fn, key=key+'/autocorr_data')
...
class x3d_correlations_xzt(spectra_base, CommonTemporalData):
    _flowstruct_class = fp.FlowStructND_time
    ...
    def _hdf_extract(self, fn, key=None):
        ...
        self.corr_data = self._flowstruct_class.from_hdf(
            fn, key=key+'/corr_data')
    ...
    def save_hdf(self, fn, mode, key=None):
        ...
        self.avg_data.save_hdf(fn, mode, key=key+'/avg_data')
        self.corr_data.to_hdf(fn, 'a', key=key+'/corr_data')
    ...
    def _get_window_params(self, path, **win_params):
        with open(os.path.join(path, 'parameters.json'), 'r') as f:
            params = json.load(f)
        if 'dt' in win_params or 'file' in win_params:
            raise ValueError("Invalid key in window_parameters")
    ...
    def _get_spectra_data(self, path, ylocs, comp='u', its=None, window_params=None):
        ...
        u_corr = np.zeros(
            (len(its), len(ylocs), self.NCL[1], self.NCL[0]), dtype='f8')

        for i, it in enumerate(its):
            fluct = self._module._fluct_xzt_class(it, path=path,
                                                  comps=comp,
                                                  avg_data=self.avg_data)
        ...
        if window_params:
            ...
            for k, win in enumerate(window_itsc):
                ...
                if window_params['method'] == 'uniform':
                    data[k] = np.mean(u_corr[win_its], axis=0)
                else:
