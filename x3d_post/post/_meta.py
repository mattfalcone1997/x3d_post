...
class line_probes(Common):
    ...
    def _extract_probe_file(self, probe_path: str, fn: str, n: int, its_req: list = None) -> dict[int, fp.FlowStructND_time]:
        ...
        for i, fn in enumerate(fn_list):
            ...
            probe_data[i+1] = fp.FlowStructND_time(
                coorddata, data[ind, :, :].reshape((shape1)), index=index, data_layout=('z'))
    ...
    def _extract_probes(self, path):
        ...
        if len(params_files) > 1:
            ...
            self.probe_data = self._extract_probe_file(
                probe_path, params_files[0], 1)
    ...
    def _get_probe_times(self, probe_path):
        ...
        its_list = [np.loadtxt(fn, skiprows=1, delimiter=',',
                               usecols=0, dtype='i4') for fn in sorted(times_files)]
    ...
    def _hdf_extract(self, fn, key=None):
        ...
        self.meta_data = self._module._meta_class.from_hdf(
            fn, key=key+'/meta_data')

        h5_obj = fp.hdfHandler(fn, 'r', key=key)
        keys = sorted([int(k.split()[-1])
                      for k in h5_obj.keys() if 'probe' in k])

        self.probe_data = {}
        self.probe_locations = {}
        for k in keys:
            self.probe_data[k] = fp.FlowStructND_time.from_hdf(
                fn, key=key+'/probe %d' % k)
